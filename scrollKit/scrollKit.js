...
class scrollKit {
	...
	* checking whether the parent container's overflow is hidden 
	*/
	constructor (parent_elem_cls_name,type){
		...
		if(this.checkParentStyle() == "hidden"){
			...
			alert("error Not hidden")
		}
		
	}
	/*
	* Getting the parent style 
	*/
	checkParentStyle(){
	...
	/*
	* setting up the scroll bar container
	* In the parent container the scroll bar container is created 
	* then scroll track is created within the scroll bar next the scroll thumb is created within the scroll track that's why 'track' variable is used here the add one with one  */
	setScrollBar(){
	...
	//vertical scroll bar
	/**
	 * disabling the default wheel and scroll event is compulsory
	 * Intializing scrollthumb,scrolltrack,parent elem ,thumb height,speed,compensation , to a local variable
	 * thumbx and y will get the current thumb's coordinate
	 * wheelX,wheelY,wheelZ defines the axis of the scroll which makes the parent_elem scrolltop
	 * current_scroll_top defines the current top from scroll top
	 * **/
	scroll_bar_(){
		...
		parent_elem.addEventListener('wheel',(e)=>{
			...
			function setScrollThumb(){
				...
				temp *= compensation;
				console.log(( (temp/speed)) +  "comp answer" );
		...
		scroll_thumb.onmousedown = (event) => {
			...
			// alert("parent element height : " + parent_elem_style_height)
	 ...
	 * -------
	 * uninit_thumb_height defines the uninitaitlized thumg's height becuase we don't know if the thumb's exceeds less than zero [here 10%]
	 * thumb_height -  resulted thumb's height
	 * speed - how much speed does it go , for example if force-overflow is 400px then the parent is 200px then for one thumb's pixel should equal to top:2 of the force-overflow
	 * occupied -  how much amt of thumb is occupied additionally, for example if the uninit thumb's height is 4 and the minimum height is 20 then 16 is occupied
	 * compensation -  1st: compensation is defines as how much pixel does it should go it is calculated by occupied x speed then we get it ! 2nd  this should divided according to each move of pixel , that can be done by total compensation / current track's balance  **/
	setThumbHeight () {
		// setting Thumb height
		// need event listners to update it

		// initialzie
		var uninit_thumb_height,thumb_height,avg_thumb_height,speed,occupied,current_track_balance_height;
		var compensation = 0;
		...
		console.log("check : "  + (((10/100) * scroll_height)/scroll_height)* parent_height)
		if(uninit_thumb_height < avg_thumb_height ){
			...
			console.log("occupied : " + occupied + "speed : "+ speed +"avg_thumb_height : " +  avg_thumb_height +"uninit_thumb_height : "+ uninit_thumb_height + "comp :" + compensation)
		...
		console.log( "avg thumg height: " +  avg_thumb_height + " Thumb height " + thumb_height + " Total : " + scroll_height + "  visible :   " + parent_height  + "speed : " + speed)
		...
		//setting 
		console.log("THUMB HEIGHT : " + thumb_height)
